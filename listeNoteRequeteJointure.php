...
$statement = $connexion->query("SELECT * FROM  eleve AS e
                              RIGHT JOIN note AS n ON n.id_eleve = e.id_eleve
                              RIGHT JOIN matiere AS m ON m.id_matiere = n.id_matiere ORDER BY m.id_matiere ASC;");
...
foreach($results as $result){
    if(array_key_exists($result["id_eleve"] , $eleves)){
      ...
      $notes[$result["id_matiere"]]  = array($result["sujet"] => $result["valeur"]) ;
